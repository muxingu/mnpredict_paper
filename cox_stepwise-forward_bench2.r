...
mltar="1"; trainids= as.character(read.table(paste0(d,"/training/",mltar,"/ids.txt"),stringsAsFactors=F)[,1])
...
od=paste0(d,"/cox_stepwise-forward_bench2/",tag2); dir.create(od,showWarnings=F,recursive=T)
od2=paste0(od,"/iter"); dir.create(od2,showWarnings=F,recursive=T)

excl=read.table(paste0(d,"/init.txt"),stringsAsFactors=F)[,1]
