...
source("/nfs/users/nfs_m/mg31/data/own/20220315_dnanexus/code/loaddata_bench2.r")

args = commandArgs(trailingOnly=TRUE)
tagty=args[1]; dis=args[2]

vars=read.table(paste0(d,"/cox_stepwise-forward_bench2/final_",dis,".txt"),stringsAsFactors=F)[,1]
...
od=paste0(d,"/cox/1",evtag,"_final_bench2/",tag); dir.create(od,showWarnings=F,recursive=T)
...
dfpred=matrix(,nrow=0,ncol=length(tartime)+1); colnames(dfpred)=c(tartime,"max")
for(j in seq(1,nrow(datftrain),1000)){
  low=j; high=min(j+999,nrow(datftrain))
  fit = survfit(cox, datftrain[low:high,vars])
  tis=closest(fit$time,tartime)
  pred=t(fit$surv[tis,])
  dfpred=rbind(dfpred,pred)
}
otab=data.frame( round(dfpred,digits=4), survtime=survtime[ctrain])
write.table(otab, file=paste0(od,"/train_",tagty,"_pred.txt"),sep="\t",col.names=NA,quote=F)
